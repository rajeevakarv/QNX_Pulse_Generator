...
void main(void)
{
  ...
  if(post_function())
  {
     ...
     (void) printf("histogram in ascending order, with the lowest arrival time for that bucket\r\n");
...
*/ 
  ...
  for (i = 0; i < numberOfBuckets; ++i)
  {
     ...
     {
       //(void)printf("histogram[%d]  %u\r\n", i, histogram[i]);
       (void)printf("MinimumValue for bucket %u  histogram[%d]  %u \r\n", minimumHistogramValueUs[i], i, histogram[i]);
       userinput = GetChar();
     }
  };
  
  (void)printf("\n\rMaximum interarrival time for input Pulse: %d \r\n", MaxInputValue);
  (void)printf("\n\rMinimum interarrival time for input Pulse: %d \r\n", MinInputValue);
...
void processTimerMeasurements(UINT16 lowerBoundaryUs, UINT16 upperBoundaryUs)
{
   ...
   for (i = 0; i < MAXINPUTVALUES - 1; ++i)
   {
      // The below if condition will set-up the Max and Min values for the given input Pulse.
      if(i ==0 ) {
         MaxInputValue = pulseIntervalsUs[i];
         MinInputValue = pulseIntervalsUs[i];
      } else {
          if (pulseIntervalsUs[i] > MaxInputValue) {
            MaxInputValue = pulseIntervalsUs[i];
          } else if (pulseIntervalsUs[i] < MinInputValue){
            MinInputValue = pulseIntervalsUs[i];
          }
